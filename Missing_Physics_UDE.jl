...
begin
	# Compute the ideal missing terms
	Ȳ = zeros(size(X))
	for i in 1:size(X, 2)
		Ȳ[1, i] = -p_[2] * X[1, i] * X[2, i]
		Ȳ[2, i] = p_[3] * X[1, i] * X[2, i]
	end
end

# ╔═╡ 5ab51fdf-bcb9-474a-8608-fc6dc96ea599
# Define symbolic basis
MTK.@variables u[1:2]

# ╔═╡ 6589b8ab-c07d-4586-b82c-6e75565b1e57
b = DataDrivenDiffEq.polynomial_basis(u, 4)

# ╔═╡ bc55e3be-3ddf-4090-ab2a-9980119be679
basis = DataDrivenDiffEq.Basis(b, u)

# ╔═╡ e00c525a-1721-4546-b266-003e9abe449f
# Create DataDriven problems
full_problem = DataDrivenDiffEq.ContinuousDataDrivenProblem(Xₙ, t)

# ╔═╡ 58f9ce24-a50e-40bc-aa04-b133c66e3804
ideal_problem = DataDrivenDiffEq.DirectDataDrivenProblem(X̂_train, Ȳ)

# ╔═╡ 3db46048-f04e-4fba-846b-8c4630cd5f0a
nn_problem = DataDrivenDiffEq.DirectDataDrivenProblem(X̂_train, Ŷ)

# ╔═╡ 6ec76cc6-ad08-498e-8374-32f144b439f5
# Setup sparse regression parameters
λ = 1e-1
...
# Define common options for solving
...
[compat]
ComponentArrays = "~0.15.49"
DataDrivenDiffEq = "~1.16.1"
DataDrivenSparse = "~0.2.2"
LineSearches = "~7.8.2"
Lux = "~1.31.4"
...
manifest_format = "2.0"
project_hash = "f355928bcef82c8dd5c71e45f51e8944aedccd16"
...
git-tree-sha1 = "682db2440a82f959268bc38c9f19ec77f0c51efa"
...
git-tree-sha1 = "ff439180ad0220138b8b1c7f6d265dec693e2f7d"
...
deps = ["CommonSolve", "CurveFit", "DataInterpolations", "DataStructures", "DiffEqBase", "DocStringExtensions", "LinearAlgebra", "MLUtils", "ModelingToolkitBase", "Parameters", "PrecompileTools", "QuadGK", "Random", "RecipesBase", "Reexport", "SciMLBase", "SciMLPublic", "SciMLStructures", "Setfield", "Statistics", "StatsAPI", "StatsBase", "SymbolicUtils", "Symbolics"]
git-tree-sha1 = "d4fa88db5047e7b21edac099375998c975715b4c"
uuid = "2445eb08-9709-466a-b3fc-47e12bd697a2"
version = "1.16.1"

[[deps.DataDrivenSparse]]
deps = ["CommonSolve", "DataDrivenDiffEq", "DocStringExtensions", "LinearAlgebra", "Parameters", "Printf", "SciMLPublic", "Statistics", "StatsAPI"]
git-tree-sha1 = "694387f0a94696c4b0aea517de899ad480036afa"
...
git-tree-sha1 = "cc280cd71e4adccbad324bcb0b58288ef4781e57"
...
[[deps.FastRationals]]
git-tree-sha1 = "95e0f61bbec4578481f6925ee9f1fc6fa508ac7b"
...
version = "9.32.4"
weakdeps = ["Adapt", "ForwardDiff", "KernelAbstractions", "OrdinaryDiffEqCore"]
