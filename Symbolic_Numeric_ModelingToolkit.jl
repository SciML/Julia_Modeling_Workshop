...
So up until this point, we still have not effectively used ModelingToolkit at all. Those are simply Symbolics expressions. Next, we impart context to it by defining a `System`:
"""
...
md"""
We will see in a second how and why this is important. So now, let's solve this ODE! To do this, we use the ODEProblem constructor. It's just like DifferentialEquations.jl, but instead of taking `ODEProblem(f,u0,tspan,p)`, it takes the system information instead of `f`, it takes a mapping from symbolic values to values (the "operating point", covering both initial conditions and parameter values) instead of `u0`, and tspan is the same. This looks like:
"""
...
prob2 = ODEProblem(sys, [🐰 => 1.0, 🐺 => 1.0, α => 3.0], (0.0,10.0))
...
manifest_format = "2.1"
project_hash = "a9c51b9a1c750ae7f7279e58c3d1573a5da8bd78"
