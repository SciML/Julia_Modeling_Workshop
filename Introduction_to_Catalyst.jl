...
# ╔═╡ 885f5685-5239-42aa-942c-2b9bdf7df229
using Random

# ╔═╡ 072b2684-ec7e-4451-83d3-6d1d31aff54d
md"""
# Introduction to Biological Modeling with Catalyst.jl

In this tutorial we provide an introduction to using Catalyst to specify chemical reaction networks, and then to solve ODE, jump, and SDE models generated from them.

We'll demonstrate the abstract mathematical models Catalyst reaction models can be converted to, starting with a simple example.
"""

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced0d
import CairoMakie, GraphMakie, NetworkLayout

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd943
md"""
## Creating a Reaction Network
...
We use a mix of first order, zero order, and repressive Hill function rate laws. Note, ∅ corresponds to the empty state, and is used for zeroth order production and first order degradation reactions:
"""

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501e4
...
# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b30d
md"""
The `@reaction_network` returns a `ReactionSystem`, which we saved in the `rn` variable. It can be converted to a variety of other mathematical models represented as `ModelingToolkit.AbstractSystem`s, or analyzed in various ways using the Catalyst.jl API.
"""

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaa3
# View the chemical species
species(rn)

# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cc3
# View the parameters
parameters(rn)

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced0e
# View the reactions
reactions(rn)

# ╔═╡ 885f5685-5239-42aa-942c-2b9bdf7df223
md"""
We can also use Latexify to see the corresponding reactions in LaTeX, which shows what the `hillr` terms mathematically correspond to:
"""

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd944
latexify(rn)

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501e5
md"""
## Network Visualization

Catalyst also has functionality for visualizing networks using the Makie plotting ecosystem. The `plot_network` function allows us to visualize the network:
"""

# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b30e
g = plot_network(rn)

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaa4
md"""
The network graph shows:
- **Blue nodes**: Chemical species
- **Green nodes**: Reactions
- **Black arrows from species to reactions**: Reactants (labeled with stoichiometry)
- **Black arrows from reactions to species**: Products (labeled with stoichiometry)
- **Red arrows from species to reactions**: Species used in rate expressions

For the repressilator, the Hill repression functions create red arrows from each protein to the corresponding reaction.
"""

# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cc4
md"""
## Mass Action ODE Models

Let's now use our `ReactionSystem` to generate and solve a corresponding mass action ODE model. We first convert the system to a ModelingToolkit `System`:
"""

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced0f
odesys = ode_model(rn)

# ╔═╡ 885f5685-5239-42aa-942c-2b9bdf7df224
md"""
Before we can solve the ODEs, we need to specify:
1. Parameter values
2. Initial conditions
3. Time interval

We can build mappings using Julia `Symbols`:
"""

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd945
# Parameter mapping
pmap = (:α => .5, :K => 40, :n => 2, :δ => log(2)/120,
        :γ => 5e-3, :β => log(2)/6, :μ => log(2)/60)

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501e6
# Initial condition mapping
u₀map = [:m₁ => 0., :m₂ => 0., :m₃ => 0., :P₁ => 20., :P₂ => 0., :P₃ => 0.]

# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b30f
# Time interval
tspan = (0., 10000.)

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaa5
md"""
Alternatively, we can use ModelingToolkit-based symbolic variables:
"""

# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cc5
psymmap = (rn.α => .5, rn.K => 40, rn.n => 2, rn.δ => log(2)/120,
           rn.γ => 5e-3, rn.β => 20*log(2)/120, rn.μ => log(2)/60)

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced10
u₀symmap = [rn.m₁ => 0., rn.m₂ => 0., rn.m₃ => 0., rn.P₁ => 20.,
            rn.P₂ => 0., rn.P₃ => 0.]

# ╔═╡ 885f5685-5239-42aa-942c-2b9bdf7df225
md"""
Now we can set up and solve the ODE problem:
"""

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd946
# Create the ODEProblem
oprob = ODEProblem(rn, u₀map, tspan, pmap)

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501e7
# Solve and plot the ODE
sol_ode = solve(oprob, Tsit5(), saveat=10.0)

# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b310
plot(sol_ode, title="Repressilator ODE Solution")

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaa6
md"""
We see the well-known oscillatory behavior of the repressilator!
"""

# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cc6
md"""
## Stochastic Simulation Algorithms (SSAs)

Let's now look at a stochastic chemical kinetics model of the repressilator, modeling it with jump processes. We'll construct a `JumpProblem` that uses Gillespie's Direct method:
"""

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced11
# Redefine initial condition to be integer valued for SSA
u₀map_int = [:m₁ => 0, :m₂ => 0, :m₃ => 0, :P₁ => 20, :P₂ => 0, :P₃ => 0]

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd947
# Create a JumpProblem (auto-selects Gillespie's Direct Method)
jprob = JumpProblem(rn, u₀map_int, tspan, pmap)

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501e8
# Solve and plot the jump process
sol_jump = solve(jprob)

# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b311
plot(sol_jump, title="Repressilator Stochastic Simulation (SSA)")

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaa7
md"""
We see that oscillations remain, but become much noisier. We can also choose a specific SSA method:
"""

# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cc7
# Using SortingDirect method explicitly
jprob_sorting = JumpProblem(rn, u₀map_int, tspan, pmap; aggregator = SortingDirect())

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced12
sol_jump_sorting = solve(jprob_sorting)

# ╔═╡ 885f5685-5239-42aa-942c-2b9bdf7df227
plot(sol_jump_sorting, title="Repressilator with SortingDirect SSA")

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd948
md"""
## Chemical Langevin Equation (CLE) SDE Models

At an intermediate physical scale between macroscopic ODE models and microscopic stochastic chemical kinetics models lies the CLE, given by a system of SDEs.

Since the repressilator has species that get very close to zero (not ideal for CLE), let's create a simpler birth-death process:
"""

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501e9
bdp = @reaction_network BirthDeath begin
    c₁, X --> 2X
    c₂, X --> 0
    c₃, 0 --> X
end

# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b312
# Parameters for birth-death process
p_bdp = (:c₁ => 1.0, :c₂ => 2.0, :c₃ => 50.)

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaa8
# Initial condition for birth-death process
u₀_bdp = [:X => 5.]

# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cc8
# Time span for birth-death process
tspan_bdp = (0., 4.)

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced13
md"""
The corresponding Chemical Langevin Equation SDE is:

$$dX(t) = \left( c_1 X(t) - c_2 X(t) + c_3 \right) dt + \sqrt{c_1 X(t)} dW_1(t) - \sqrt{c_2 X(t)} dW_2(t) + \sqrt{c_3} dW_3(t)$$

where each $W_i(t)$ denotes an independent Brownian Motion.
"""

# ╔═╡ 885f5685-5239-42aa-942c-2b9bdf7df228
# Create SDEProblem for CLE
sprob = SDEProblem(bdp, u₀_bdp, tspan_bdp, p_bdp)

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd949
# Solve and plot the SDE
sol_sde = solve(sprob, LambaEM(), tstops = range(0., step = 4e-3, length = 1001))

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501ea
plot(sol_sde, title="Birth-Death Process via Chemical Langevin Equation")

# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b313
md"""
## Specifying Complete Models via the DSL

Catalyst also supports specifying default values for parameters and initial conditions directly in the DSL:
"""

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaa9
bdp2 = @reaction_network BirthDeath2 begin
...
# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cc9
# Create SDEProblem with defaults - no need to specify parameters or initial conditions
sprob2 = SDEProblem(bdp2, [], tspan_bdp)

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced14
md"""
Let's simulate both models with the same random seed to verify they give identical results:
"""

# ╔═╡ f5c9ea83-6dba-4848-8340-1a8c772bd94a
begin
    # First simulation with seed
    Random.seed!(1)
    sol_sde1 = solve(sprob, LambaEM(), tstops = range(0., step = 4e-3, length = 1001))
    p1 = plot(sol_sde1, title="Original Model", legend=false)
end

# ╔═╡ e13c7d4f-4cdb-43d5-9f77-33401cb501eb
begin
    # Second simulation with same seed
    Random.seed!(1)
    sol_sde2 = solve(sprob2, LambaEM(), tstops = range(0., step = 4e-3, length = 1001))
    p2 = plot(sol_sde2, title="DSL with Defaults", legend=false)
end

# ╔═╡ c7193526-d32f-4f2a-95aa-09e98b88b314
plot(p1, p2, layout = (2,1))

# ╔═╡ 36985121-9fe5-4961-9ec2-3f5ee57cfaaa
md"""
## Reaction Rate Laws in Simulations

In generating mathematical models from a `ReactionSystem`, reaction rates are treated as *microscopic* rates. For a general mass action reaction of the form $n_1 S_1 + n_2 S_2 + \dots + n_M S_M \to \dots$ with stoichiometric substrate coefficients $\{n_i\}_{i=1}^M$ and rate constant $k$:

**ODE and SDE rate laws:**
$$k \prod_{i=1}^M \frac{(S_i)^{n_i}}{n_i!}$$

**Jump process transition rate (propensity):**
$$k \prod_{i=1}^M \frac{S_i (S_i-1) \dots (S_i-n_i+1)}{n_i!}$$

For example, the reaction $2X + 3Y \to Z$ with rate constant $k$ has rate law:
...
# ╔═╡ 45dc0462-d63c-11ef-0f62-e5d029618cca
md"""
This implicit rescaling can be disabled using `combinatoric_ratelaws=false`:

```julia
ode_model(rn; combinatoric_ratelaws=false)
```

This would give the rate law $k X^2 Y^3$ instead.
"""

# ╔═╡ 6d29e44b-3d70-4700-ad53-9e75df7ced15
md"""
## Summary

In this tutorial, we've demonstrated:

1. **Creating reaction networks** using the `@reaction_network` macro
2. **Visualizing networks** with Makie plotting
3. **Solving ODE models** for deterministic dynamics
4. **Running stochastic simulations** using SSAs (Gillespie methods)
5. **Solving SDE models** using the Chemical Langevin Equation
6. **Specifying complete models** with default parameters in the DSL
7. **Understanding rate laws** and their mathematical forms

Catalyst provides a unified interface for working with chemical reaction networks across multiple modeling paradigms, from deterministic ODEs to stochastic jump processes and SDEs.
"""
...
manifest_format = "2.0"
project_hash = "b6cfa6fcb85f949be12f8301a9b1c33a81a8823a"
...
git-tree-sha1 = "ab00b6e90a5fe0999bc1d3ff91ec4d4287bb2b6f"
uuid = "479239e8-5488-4da2-87a7-35f2df7eef83"
version = "16.4.3"

    [deps.Catalyst.extensions]
    CatalystBifurcationKitExtension = "BifurcationKit"
    CatalystCairoMakieExtension = ["CairoMakie", "Makie"]
    CatalystGraphMakieExtension = ["GraphMakie", "NetworkLayout", "Makie"]
    CatalystHomotopyContinuationExtension = ["DynamicPolynomials", "HomotopyContinuation", "TermInterface"]
...
deps = ["DiffEqBase", "FastBroadcast", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "Reexport", "SciMLBase", "StochasticDiffEqCore"]
git-tree-sha1 = "368102847deae1d2e94a7a256849bae2f1312a69"
...
git-tree-sha1 = "fadc770c5afe7a331480c1550bc2728f661f91b2"
...
git-tree-sha1 = "253198f8c44b44f3112c5ec4f5a3b180f1c77155"
...
deps = ["DiffEqBase", "DiffEqNoiseProcess", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "RecursiveArrayTools", "Reexport", "SciMLBase", "StaticArrays", "StochasticDiffEqCore"]
git-tree-sha1 = "443030d5fb11be7aced9a5b7fa74ac544d701e47"
...
git-tree-sha1 = "a49ed4ffee8263beec68d6f3ae3044e6d334ae34"
uuid = "8c95a807-c8e7-4581-8419-890d001af53e"
version = "2.1.1"

[[deps.StochasticDiffEqROCK]]
deps = ["DiffEqBase", "LinearAlgebra", "MuladdMacro", "OrdinaryDiffEqCore", "Random", "RecursiveArrayTools", "Reexport", "SciMLBase", "StaticArrays", "StochasticDiffEqCore"]
git-tree-sha1 = "5d3a8ce9c2c64a2f8e12900722230456cbadceae"
...
git-tree-sha1 = "15722ebc298724f21ed0de14ed4812fd861f0af4"
...
git-tree-sha1 = "7a7a17c5922b00618bd6ae44af041f6114bfd4da"
